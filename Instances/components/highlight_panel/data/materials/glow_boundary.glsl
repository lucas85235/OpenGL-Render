...
highp vec4 hoverAndSelectPanelMask(highp vec2 fragCoord, highp vec4 myPanelColor, highp vec4 myBorderColor,
highp vec2 cursorPosition, bool isSelected, highp float lerpTime, highp float controlSelectEffect,
highp float changeStateLerp, highp float circleRadius) {
    highp vec2 posCircle = fragCoord;
    //    highp vec2 circleCenter = vec2(cursorPosition.x * 0.7, cursorPosition.y * 0.6); // Magic number for SysUI
    highp vec2 circleCenter = vec2(cursorPosition.x * 0.45, cursorPosition.y * 0.45); // Magic number for Sample todo: remove magic numbers
...
highp float verticalLineDistanceFunction(in highp vec2 pos, in highp vec2 rectSize, in highp float cornerRadius,
highp float borderThickness, highp float linePercent) {
...
highp float horizontalLineDistanceFunction(in highp vec2 pos, in highp vec2 rectSize,
in highp float cornerRadius, highp float borderThickness, highp float linePercent) {
    if (linePercent <= 0.0001)
    return 0.0;
...
highp float cornerArcDistanceFunction(in highp vec2 pos, in highp vec2 rectSize,
in highp float cornerRadius, in highp float borderThickness, in highp float arcPercent) {
...
highp float roundedCornerRectBorderMask(in highp vec2 inPanelWorldSpaceCoords, in highp vec2 panelRectSize,
highp float cornerRadius, highp float borderThickness, highp float lineSpread,
