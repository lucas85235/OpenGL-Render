...
// create a gradient between 5 color by gray scale color texture
vec3 gradientGrayScaleTexture(vec3 grayTextureColor, vec3 color1, vec3 color2, vec3 color3, vec3 color4, vec3 color5)
{
    highp vec3 t1 = smoothstep(0.0, 0.25, grayTextureColor.rgb);
    highp vec3 t2 = smoothstep(0.25, 0.5, grayTextureColor.rgb);
    highp vec3 t3 = smoothstep(0.5, 0.75, grayTextureColor.rgb);
    highp vec3 t4 = smoothstep(0.75, 1.0, grayTextureColor.rgb);

    highp vec3 color = mix(color1, color2, t1);
