...
class [[deprecated("Use IFileSystem and VFS instead.")]] FileSystem {
public:
  // Retorna o caminho absoluto correto para um arquivo
  static std::string GetPath(const std::string &path) {
    // 1. Tenta relativo ao executável (Pasta Build/Release)
    // Se o CMake copiou a pasta "shaders" para "build/shaders", isso funciona
    // direto.
    if (fs::exists(path)) {
      return path;
    }
    ...
    // Se falhar, retorna o original e deixa o erro acontecer no log do
    // Shader/Texture std::cerr << "[FS] Arquivo não encontrado: " << path <<
    // std::endl;
    return path;
  }
